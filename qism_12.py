...
class Game:
        ...
        for bullet in self.tank.bullets:
            for enemy in self.enemies:
                if bullet.active and bullet.get_rect().colliderect(enemy.get_rect()):
                    bullet.active = False
                    self.tank.booms.append(Boom(enemy.x + enemy.width // 2, enemy.y + enemy.width // 2))  # "BOOM" yozuvini qo'shish
    ...
    def activate_rocket(self):
        if self.enemies:  
            ...
            rocket = Rocket(self.tank.x, self.tank.y, first_enemy.x, first_enemy.y)
            self.tank.bullets.append(rocket)  
            
    def run(self):
        while self.running:
            # orqa fon e'lon qilish
